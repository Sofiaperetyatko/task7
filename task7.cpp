...
//этот массив нужен чтобы вставить слова одной строки в него,потом сравнивать с другой строкой
{
...
void print_massive_char(const char* word, char* array, int& n)// функция,чтобы разделить строку на слова
{
	cout << word;
	int i = 0;
	do
...
//находим длину слов в строке
{
	...
	for (int i = 0; i < size; ++i)
	{
		if (dmass[i] != s[0] && dmass[i] != s[1] && dmass[i] != s[2] && dmass[i] != s[3] && dmass[i] != s[4])// если элемент
	//в двумерном массиве(строке) не равен одному из этих символов,то отправляем его в одномерный массив,чтобы похже вывести строку по слову
...
void look_matix(char** matrix, int size, int* len_word)// функция для выводы каждого слова 
{
	for (int i = 0; i < size; ++i)
	{
...
int main()
{
    setlocale(LC_ALL, " "); //чтобы выодились русские буквы в консоли
